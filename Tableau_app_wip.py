    ...
    if st.button("Connect to Tableau and Fetch Content Info"):
        if token_name and token_value and server_url:
            ...
            if workbooks_df is not None:
                ...
                st.download_button(
                    label="Download Workbooks Info",
                    data=workbooks_df.to_csv(index=False).encode(),
                    file_name="workbooks_info.csv",
                    mime="text/csv"
                )
                ...
                st.download_button(
                    label="Download Datasources Info",
                    data=datasources_df.to_csv(index=False).encode(),
                    file_name="datasources_info.csv",
                    mime="text/csv"
                )
                ...
                st.download_button(
                    label="Download Projects Info",
                    data=projects_df.to_csv(index=False).encode(),
                    file_name="projects_info.csv",
                    mime="text/csv"
                )
                ...
                st.download_button(
                    label="Download Views Info",
                    data=views_df.to_csv(index=False).encode(),
                    file_name="views_info.csv",
                    mime="text/csv"
                )
            else:
