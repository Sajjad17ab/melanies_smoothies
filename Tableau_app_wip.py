...
# Radio button to switch between create project, content info, publish workbook, create group, and download workbook
...
# If the user selects "Download Workbook"
    ...
    # Ask for the workbook ID to download
    ...
    if st.button("Download Workbook"):
        if workbook_id:
            try:
                ...
                with server.auth.sign_in(tableau_auth):
                    # Fetch the workbook by its ID
                    workbook = server.workbooks.get_by_id(workbook_id)
                    if workbook:
                        # Download the workbook as a .twb or .twbx file
                        file_path = f"/tmp/{workbook.name}.twbx"
                        
                        # Download the workbook to the specified path
                        server.workbooks.download(workbook, file_path)
                        ...
                        st.error("Workbook with the provided ID not found.")
            except Exception as e:
