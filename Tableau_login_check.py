        ...
        if not token_name or not token_value or (not server_url and not cloud_url) or not project_name:
            ...
            try:
                ...
                with server.auth.sign_in(tableau_auth):
                    st.write("Successfully authenticated with Tableau!")

                    # Get all projects for debugging purposes
                    all_projects, pagination_item = server.projects.get()
                    st.write(f"Found {len(all_projects)} projects.")
                ...
                st.write(f"Server URL: {server_url if connection_type == 'Tableau Server' else cloud_url}")
